...
//================================
    ...
    mysql_init(&mysql_remote);
    if(!mysql_real_connect(&mysql_remote, "isce.ncsa.illinois.edu", "isc", NULL, "isc", 0, NULL, 0)){
        printf("ERROR connecting to remote database\n\nExiting\n");
        exit(3);
    }

    mysql_init(&mysql_remote2);
    if(!mysql_real_connect(&mysql_remote2, "isct.ncsa.illinois.edu", "isc", NULL, "isc", 0, NULL, 0)){
        printf("ERROR connecting to remote database\n\nExiting\n");
        exit(3);
    }
...
//printf("Q\n");
		...
		i = mysql_query(&mysql_remote2, sqlbuf);
		//printf ("Query: '%s'\n", sqlbuf);
	}
...
//              printf ("Query: '%s'\n", sqlbuf);
	...
	//check to see if these lines are still needed.  may be redundant.
	i = mysql_query(&mysql, sqlbuf);
	i = mysql_query(&mysql_remote, sqlbuf);
	i = mysql_query(&mysql_remote2, sqlbuf);
